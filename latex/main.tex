...
\begin{itemize}
  ...
  \item Khả năng tưới tự động tích hợp ngay trên vi điều khiển ESP32, tự quyết định thời điểm tưới dựa trên cấu hình từ xa.
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5cm]
  ...
  \item \textbf{Truyền dữ liệu qua MQTT:} ESP32 đóng gói dữ liệu thành JSON và publish lên topic \code{iot/farm/env} qua HiveMQ Cloud TLS (port 8883). ID thiết bị được tự động tạo duy nhất từ địa chỉ MAC của chip để tránh xung đột trên MQTT Broker.
  \item \textbf{Xử lý dữ liệu bằng Node-RED:} Nhận message từ MQTT, parse JSON, tách luồng hiển thị gauge, chart, theo dõi các cảnh báo.
  \item \textbf{Hiển thị Dashboard:} Gauge độ ẩm đất, gauge nhiệt độ, gauge độ ẩm không khí, biểu đồ line theo thời gian thực và thông báo trạng thái hoạt động của chế độ tự động.
  \item \textbf{Cảnh báo đa tầng:} Các toast cảnh báo thân thiện có mã màu CSS tự thiết kế riêng dựa trên các ngưỡng của ánh sáng, độ ẩm và nhiệt độ.
  \item \textbf{Điều khiển \textendash{} Cấu hình từ Dashboard:} Bật/tắt máy bơm thủ công, bật công tắc tưới tự động. Config này được truyền qua topic \code{iot/farm/config}.
  \item \textbf{Chế độ tưới tự động độc lập:} ESP32 tự kiểm tra độ ẩm (khi chế độ auto được bật) và tự đóng relay khởi động bơm 10 giây nếu đất dưới 30\%, sau đó tự ngắt (sử dụng non-blocking timer với millis).
...
Tầng thứ hai là \textbf{tầng mạng (Network Layer)}: HiveMQ Cloud nhận message từ ESP32 qua topic \code{iot/farm/env} (dữ liệu cảm biến) và phân phối cho Node-RED; đồng thời nhận lệnh điều khiển từ Node-RED qua topic \code{iot/farm/control} và \code{iot/farm/config}.

Tầng thứ ba là \textbf{tầng ứng dụng (Application Layer)}: Node-RED nhận dữ liệu từ MQTT, xử lý thành số thập phân có làm tròn (toFixed), thêm mã CSS cho các thông báo Toast và gửi lệnh điều khiển.
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5cm]
  ...
  \item Dashboard cập nhật tất cả các giá trị đo được theo thời gian thực. Lợi thế là các chỉ số đều được làm tròn (1 chữ số sau dấu phẩy).
\end{enumerate}
...
\textbf{Luồng 3 — Điều khiển và Tưới tự động:}

\begin{enumerate}[label=\arabic*., leftmargin=1.5cm]
  \item Người dùng nhấn nút \textbf{Bật/Tắt bơm} hoặc \textbf{Bật/Tắt chế độ Auto}.
  \item Node-RED gửi JSON \code{\{"pump":true\}} hoặc \code{\{"auto":true\}} tùy theo nút nhấn vào topic \code{iot/farm/control} hoặc \code{iot/farm/config}.
  \item ESP32 cấu hình cho chế độ tương ứng. Khi Auto Mode được bật, \code{loop()} của ESP32 tự kiểm tra xem đất có khô không (nhỏ hơn 30\%). Nếu có, module sẽ lập tức thiết lập \code{pumpAutoOffTime = millis() + 10000} (tưới 10 giây). Việc tưới tự động hoàn toàn không làm đứng chương trình do dùng \code{millis()}, và độc lập với Node-RED trong việc giám sát.
\end{enumerate}
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5cm]
  \item \textbf{Hệ thống IoT ba tầng hoàn chỉnh:} Tầng thiết hiện tại (ESP32 + DHT22 + BH1750 + Potentiometer + Relay) kết nối với tầng mạng (HiveMQ Cloud TLS) và tầng ứng dụng (Node-RED Dashboard), hoạt động ổn định trong điều kiện thực tế.
  ...
  \item \textbf{Điều khiển bơm thực sự qua relay:} Chế độ bật/tắt thủ công qua nút bấm Dashboard và chế độ tưới tự động dựa trên ngưỡng độ ẩm đất, với trạng thái relay kết hợp các config auto hiển thị rõ ràng, mang lại sự linh hoạt cao trong vận hành.
