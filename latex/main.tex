...
\usepackage{enumitem}
\usepackage{titlesec}
\titleformat{\section}{\centering\fontsize{15}{18}\selectfont\bfseries\uppercase}{\thesection.}{1em}{}
\titleformat{\subsection}{\fontsize{13}{16}\selectfont\bfseries}{\thesubsection.}{1em}{}
\usepackage{tocloft}
\usepackage{fancyhdr}
\usepackage{xcolor}
\usepackage{listings}
...
% Cấu hình thụt đầu dòng cho mọi đoạn văn
...
% Không in Danh mục hình vẽ / Danh mục bảng biểu (tắt hoàn toàn nếu có \listoffigures / \listoftables)
...
\begin{lstlisting}[language=bash, basicstyle=\ttfamily\small, frame=none, numbers=none, caption={Cài đặt Node-RED và node-red-dashboard}, label={lst:nr}]
...
\begin{lstlisting}[language=bash, basicstyle=\ttfamily\small, frame=none, numbers=none, caption={Chạy Node-RED}, label={lst:nrrun}]
node-red
\end{lstlisting}
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5cm]
  \item \textbf{Hệ thống IoT ba tầng hoàn chỉnh:} Tầng thiết bị (ESP32 + DHT22 + BH1750 + Potentiometer + Relay) kết nối với tầng mạng (HiveMQ Cloud TLS) và tầng ứng dụng (Node-RED Dashboard), hoạt động ổn định trong điều kiện thực tế.
