...
Lưu ý: các topic được đặt theo chuẩn để dễ mở rộng. Trên ESP32, client dùng \code{WiFiClientSecure} với TLS (cổng \code{MQTT_PORT} = 8883), \code{PubSubClient} publish/subscribe tương ứng (\file{main.cpp}).

\subsection{Xử lý dữ liệu bằng Node-RED}

Node-RED là công cụ lập trình flow-based; dữ liệu từ ESP32 được mô tả chi tiết trong \file{node-red.json}. Message MQTT từ topic \code{iot/farm/env} vào node \textbf{mqtt in} rồi phân nhánh song song.
...
\begin{figure}[H]
  ...
  \caption[Sơ đồ flow xử lý dữ liệu trên Node-RED]{Sơ đồ luồng xử lý dữ liệu Node-RED — AgriSense IoT: MQTT \code{iot/farm/env} phân nhánh gauge, chart, cảnh báo và điều khiển \code{iot/farm/control}}
  \label{fig:flow}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption[Giao diện tổng quan hiển thị dữ liệu trên Node-RED Dashboard]{Giao diện Node-RED Dashboard AgriSense: giám sát đất và tưới, không khí, ánh sáng; nút bật/tắt bơm và tưới tự động; ví dụ toast cảnh báo ánh sáng yếu (\texttt{127.0.0.1:1880/ui})}
  \label{fig:dashboard-1}
\end{figure}

\subsection{Cảnh báo môi trường}

Trên Dashboard: toast khi \code{temperature > 35}°C, khi \code{soilMoisture < 30\%} (đất khô), khi \code{soilMoisture < 20\%} (nguy hiểm), và khi \code{lightLux < 1000} (thiếu sáng) (\file{node-red.json}). Trên ESP32: buzzer phát bíp 2 lần khi \code{soilMoisture < 30\%}, phát bíp 5 lần khi \code{soilMoisture < 20\%} trong \code{loop()} (\file{main.cpp}).

\begin{figure}[H]
  \centering
  % Ảnh minh họa riêng cảnh báo (toast) — có thể dán crop từ Dashboard
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.88\textheight,keepaspectratio]{images/nodered-dashboard-full.png}
  \caption[Giao diện nút nhấn và công tắc điều khiển máy bơm từ xa]{Giao diện Dashboard AgriSense: khối \textit{Điều khiển} với nút Bật/Tắt bơm và công tắc Tưới tự động (\texttt{127.0.0.1:1880/ui})}
  \label{fig:dieukhien}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.88\textheight,keepaspectratio]{images/mach-vat-ly-wokwi.png}
  \caption[Mạch mô phỏng Wokwi]{Mạch mô phỏng Wokwi: ESP32 DevKit C v4 + DHT22 + BH1750 + Potentiometer + Relay + Buzzer (khớp \file{diagram.json})}
  \label{fig:wokwi}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption[Sơ đồ nguyên lý phần cứng (EasyEDA)]{Sơ đồ nguyên lý phần cứng (EasyEDA): ESP32, DHT22, Potentiometer, Relay, Buzzer (cảm biến ánh sáng BH1750 được bổ sung trên mạch mô phỏng Wokwi)}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth,height=0.45\textheight,keepaspectratio]{images/esp32.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth,height=0.45\textheight,keepaspectratio]{images/dht22.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth,height=0.45\textheight,keepaspectratio]{images/bh1750.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth,height=0.45\textheight,keepaspectratio]{images/potentiometer.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth,height=0.45\textheight,keepaspectratio]{images/relay.png}
  \caption[Module Relay điều khiển máy bơm nước]{Module Relay điều khiển máy bơm nước}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth,height=0.45\textheight,keepaspectratio]{images/buzzer.png}
  \caption[Còi Buzzer phát âm thanh cảnh báo]{Còi Buzzer phát âm thanh cảnh báo}
...
\begin{figure}[H]
  ...
  \caption[Lưu đồ thuật toán trên ESP32 (phần setup và loop)]{Lưu đồ thuật toán trên ESP32 (phần setup và loop)}
  \label{fig:luudo}
\end{figure}
...
\begin{figure}[H]
  \centering
  % Ảnh sẽ được dán vào sau
  \caption[Lưu đồ thuật toán trên ESP32 (phần đọc cảm biến, cảnh báo, MQTT)]{Lưu đồ thuật toán trên ESP32 (phần đọc cảm biến, cảnh báo, MQTT)}
  \label{fig:luudo-2}
\end{figure}
...
\begin{figure}[H]
  \centering
  % Ảnh sẽ được dán vào sau
  \caption[Giao diện Dashboard Node-RED (phần 2/2): nhiệt độ, điều khiển, độ ẩm đất (gauge, biểu đồ line, trạng thái đất và cảnh báo)]{Giao diện Dashboard Node-RED (phần 2/2): nhiệt độ, điều khiển, độ ẩm đất (gauge, biểu đồ line, trạng thái đất và cảnh báo)}
  \label{fig:nr-dash-2}
\end{figure}
...
\begin{figure}[H]
  \centering
  % Ảnh sẽ được dán vào sau
  \caption[Mô phỏng sơ đồ kết nối phần cứng thực tế trên môi trường Wokwi]{Mô phỏng sơ đồ kết nối phần cứng thực tế trên môi trường Wokwi}
  \label{fig:res-wokwi}
\end{figure}

\subsection{Kết quả truyền nhận dữ liệu qua Serial Monitor}
Trong quá trình vận hành, vi điều khiển ESP32 liên tục gửi các thông tin trạng thái hoạt động lên cửa sổ Serial Monitor. Các thông tin bao gồm quá trình kết nối Wi-Fi, trạng thái thiết lập kết nối bảo mật TLS với HiveMQ Cloud và dữ liệu cảm biến thu thập được trước khi đóng gói JSON. Đoạn mã sau trích từ \file{main.cpp} --- đúng các lệnh \texttt{Serial} tạo nên log tương ứng trên màn hình (Hình~\ref{fig:res-serial}).

% Trích đúng dòng nguồn từ src/main.cpp (Wi-Fi, MQTT, BH1750, setup, khối log + publish)
\IfFileExists{../src/main.cpp}{%
  \lstinputlisting[
    style=ccode,
    caption={Các hàm và đoạn \texttt{loop} in log lên Serial Monitor — trích \file{src/main.cpp}},
    label={lst:serial-log},
    language=C++,
    linerange={169-205,254-303,353-384}
  ]{../src/main.cpp}%
}{%
  \textbf{Lỗi:} Không tìm thấy \file{../src/main.cpp}. Biên dịch trong thư mục \file{latex/} và đảm bảo cấu trúc thư mục dự án đầy đủ.
}
...
Kết quả quan trọng nhất của hệ thống là giao diện tương tác người dùng hiện đại và trực quan. Hệ thống Dashboard cung cấp đầy đủ các thông tin cần thiết về môi trường theo thời gian thực (Hình~\ref{fig:res-dash-1} và Hình~\ref{fig:res-dash-2}).

\begin{figure}[H]
  \centering
  % Ảnh sẽ được dán vào sau
  \caption[Giao diện Dashboard (Phần 1): Hiển thị đồng hồ đo và các nút điều khiển trung tâm]{Giao diện Dashboard (Phần 1): Hiển thị đồng hồ đo và các nút điều khiển trung tâm}
...
\begin{figure}[H]
  \centering
  % Ảnh sẽ được dán vào sau
  \caption[Giao diện Dashboard (Phần 2): Biểu đồ lịch sử dữ liệu và các thông tin trạng thái đất]{Giao diện Dashboard (Phần 2): Biểu đồ lịch sử dữ liệu và các thông tin trạng thái đất}
...
\begin{table}[H]
  ...
  \label{tbl:chucnang}
  \begin{tabular}{|L{3.5cm}|C{2.5cm}|C{5cm}|}
    \hline
